...
## 功能說明
提供安全的管理後台登入頁面與 JWT 憑證管理。管理員登入後簽發 `HttpOnly` Cookie，並使用 Next.js Proxy 進行路由保護。
...
| `src/proxy.ts` | Next.js 全域路由代理與 JWT 驗證 |
...
2. **在 Proxy 發生 Node.js Crypto 模組錯誤？**
   - 由於 Next.js Proxy 執行於 Node.js 環境，本系統使用純 JavaScript 解碼函式 (`atob`) 進行驗證，以確保最大相容性。
