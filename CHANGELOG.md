...
### Added（新功能）
- **高階 Excel 報表匯出 (ExcelJS)**：導入 `exceljs` 原生二進位 `.xlsx` 產出引擎，包含頂部 Title 橫幅、4 組核心 KPI 摘要卡片、凍結視窗 (Freeze Panes)、斑馬紋與柔和彩標。
- **場次流水號自動編號**：問卷提交時自動依據日期場次生成流水編號（如 `20260723-A01`）。

### Fixed（修復）
- **控制台啟動與隱藏視窗優化**：`control.bat` 支援 PowerShell 隱藏視窗啟動，並增加 Port 3001 `EADDRINUSE` 自動自癒清理機制。
